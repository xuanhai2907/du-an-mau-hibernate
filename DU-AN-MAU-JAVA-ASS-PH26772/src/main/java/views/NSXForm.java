...
public class NSXForm extends javax.swing.JFrame {
    ...
    public void showData(List<NSXCustom> list){
        dtm.setRowCount(0);
        for (NSXCustom qlnsx : list) {
            dtm.addRow(qlnsx.toRowData());
        }
    }
    
    public void fillData(int index){
        NSXCustom ql = listQLNSX.get(index);
        lbID.setText(String.valueOf(ql.getId()));
        txtMa.setText(ql.getMa());
        txtTen.setText(ql.getTen());
    }
  
    public NSXCustom Mouse(){
    ...
    private void btnSUaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSUaActionPerformed
        ...
        NSXCustom qlcv = listQLNSX.get(chon);
        NSXCustom n = check();
        if (n == null) {
            return;
        }
